...
public class BookingController {
    ...
    @GetMapping
    public Collection<BookingDto> getAllBooking(@RequestHeader(value = USER_ID) Long userId, @RequestParam(defaultValue = "ALL") State state) {
    ...
    @GetMapping("/owner")
    public Collection<BookingDto> getAllBookingForItems(@RequestHeader(value = USER_ID) Long userId, @RequestParam(defaultValue = "ALL") State state) {
        log.info("Получен запрос 'Get /bookings/owner'");
        checkUser(userId);
        return bookingService.getAllBookingForItems(userId, state);
    }
